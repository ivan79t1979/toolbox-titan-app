...
import { Loader2, Download, Upload, Percent, FileSymlink, Wand2 } from 'lucide-react';
...
export function ImageCompressorForm() {
  ...
  const [compressed, setCompressed] = useState<{ src: string; size: number } | null>(
    null
  );
      ...
      return new Promise<void>((resolve) => {
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            // The quality parameter only applies to JPEG images.
  ...
  async function onSubmit(values: FormValues) {
    if (values.image instanceof File) {
      await compressImage(values.image, values.quality);
    }
  }
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="flex items-center gap-2">
                          <Percent className="h-4 w-4" /> Quality:{' '}
